...
; (function (root, factory) {
    ...
    MyPlugin.prototype = {
        ...
        init: function (elem, opts) {
            ...
            me.defaults = {
                ...
                operation: {
                    enable: false, // 是否启用
                    mode: "dot", // 显示方式，默认dot(可选)。值： dot 显示三个点元素(点击3个点后再显示操作按钮), direct 直接显示
        ...
        addSonMenu: function (elMenu, oneRowData) {
            ...
            if (me.settings.operation.enable) {
                ...
                treeEventHelper.doneTreeOperationEvent(me, newLiNode);
            }
        },
        ...
        if (datasource.length > 0) {
            ...
            // 初始化菜单时
                ...
                if (me.settings.operation.enable) {
                    ...
                    treeEventHelper.doneTreeOperationEvent(me, null);
                }
            }
    ...
    var treeEventHelper = {
         ...
         * @param {Object} ps_li_node 当前操作的菜单li节点，默认null(可选)
         */
        doneTreeOperationEvent: function (me, ps_li_node) {
            console.log('当前LI-node：', ps_li_node); // testing
            var _this = this;
            // 先创建增删改浮层，再触发打开增删改窗口
            if (me.settings.operation.mode == '' || me.settings.operation.mode == 'dot') {
                var manageClassName = 'treeview__menu_manage';
                var operNodeList = document.getElementsByClassName('treeview__menu_operate');
                console.log('operNodeList1-x：', operNodeList);
                Array.from(operNodeList).forEach(function (node) {
                    node.addEventListener('click', function () {
                        var parentNode = node.parentNode;
                        var nowMenuNode = parentNode; // 当前菜单Menu节点
                        ...
                        console.log('manageNode：', manageNode);
            ...
            // 直接打开增删改窗口
                ...
                console.log('operNodeList2-x：', operNodeList);
                _this.fnTreeItemManage(me, ps_li_node);
            }
        },
         ...
         * 树型菜单增删改按钮
         * @param {Object} me 当前插件对象
         * @param {Object} liNode 当前操作的菜单li节点，默认null(可选)
         */
        fnTreeItemManage: function (me, liNode) {
            console.log('liNode：', liNode);
            // var $addBtnList = document.getElementsByName('treeManageButton');
            var $addBtnList = liNode == null ? document.getElementsByName('treeManageButton') : utils.getChildrenElement(liNode, 'tree_btn');
            // testing
            console.log('$addBtnList：', $addBtnList);
